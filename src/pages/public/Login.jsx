import { AlertCircle, Circle, Eye, LoaderCircle, ShieldCheck } from "lucide-react"
...
const Login = () => {
  ...
  const { email, setEmail, password, setPassword, view, setView, isLoading, errMessage, successMessage, setSuccessMessage, setErrMessage, handleSignin, setCurrentUser } = useContext(AppContext)
  ...
  const handleLogin = async (e) => {
    ...
    if (response) {
      ...
      navigate(response.role === 'seller' ? '/seller/dashboard' : response.role === 'buyer' ? '/buyer/dashboard' : '/admin/dashboard')
    }
  }
      ...
      <div className="flex h-full">
        ...
        <div className="hidden flex-col py-12 px-15 bg-slate-900 gap-20 flex-1 lg:flex">
          ...
          <div className="flex flex-col gap-5 max-w-90">
            <p className="text-emerald-500 font-bold uppercase">Welcome Back</p>
            <h4 className="text-white text-5xl tracking-wider leading-tight font-medium">Sign in to your account</h4>
            <p className="text-slate-400">Track every deal you're holding funds for, and pick up right where you left off.</p>
          </div>
          <hr />
          ...
          <div className="w-full max-w-100 flex flex-col justify-center">
            <div className="flex flex-col gap-7 w-full">
              ...
              <div className="flex flex-col gap-2">
                ...
                <p className="text-sm text-slate-600">Enter your details to access your dashboard</p>
              </div>
              {/* Form */}
              ...
              <form onSubmit={handleLogin} method='POST' className="flex flex-col gap-3">
                ...
                <div className="flex flex-col gap-2">
                  <div className="flex items-center justify-between">
                    ...
                    <Link to='/reset-password' className="text-slate-800 text-sm hover:text-slate-700" onClick={()=> {setErrMessage(''); setSuccessMessage('')}}>Forgot password?</Link>
                  </div>
                ...
                <button type="submit" className="flex items-center justify-center bg-slate-900 text-white font-medium p-2 rounded-xl flex-1 hover:bg-slate-800 disabled:cursor-not-allowed disabled:opacity-60" disabled={isLoading}>{isLoading ? <LoaderCircle size={20} className="animate-spin" /> : 'Sign in'}</button>
              </form>
              <p className="flex items-center gap-1 text-slate-500">New to Escrow? <Link to='/register' className="text-emerald-500 font-bold" onClick={()=> {setErrMessage(''); setSuccessMessage('')}}>Create an account</Link></p>
            </div>
          </div>
        </div>
      </div>
